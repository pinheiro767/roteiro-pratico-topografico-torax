const ITEMS=window.ATLAS_ITEMS;let db,filter='all',deferredPrompt=null,audioMode=false;const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];
function esc(s){return String(s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}function slug(s){return s.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
function toast(s){const t=$('#toast');t.textContent=s;t.classList.add('show');clearTimeout(toast.t);toast.t=setTimeout(()=>t.classList.remove('show'),2300)}
function hexRgb(hex){hex=hex.replace('#','');if(hex.length===3)hex=hex.split('').map(c=>c+c).join('');const n=parseInt(hex,16);return`${(n>>16)&255},${(n>>8)&255},${n&255}`}function lum(hex){const a=hex.replace('#','').match(/.{2}/g).map(x=>parseInt(x,16)/255).map(v=>v<=.03928?v/12.92:((v+.055)/1.055)**2.4);return .2126*a[0]+.7152*a[1]+.0722*a[2]}
function theme(accent,bg){document.documentElement.style.setProperty('--accent',accent);document.documentElement.style.setProperty('--accent-rgb',hexRgb(accent));document.documentElement.style.setProperty('--bg',bg);const dark=lum(bg)<.30;[['--panel',dark?'#0d1c2b':'#ffffff'],['--panel2',dark?'#122638':'#f0f6fa'],['--ink',dark?'#f4f8fb':'#10202d'],['--muted',dark?'#a9bdca':'#657988'],['--line',dark?'#233c50':'#d7e2e9']].forEach(([a,b])=>document.documentElement.style.setProperty(a,b));try{localStorage.setItem('atlasTheme',JSON.stringify({accent,bg}))}catch(e){}}
try{const t=JSON.parse(localStorage.getItem('atlasTheme')||'null');if(t)theme(t.accent,t.bg)}catch(e){}
$('#paletteBtn').onclick=()=>$('#palette').hidden=!$('#palette').hidden;$('#accessBtn').onclick=()=>$('#accessPanel').hidden=!$('#accessPanel').hidden;$('#accent').oninput=e=>theme(e.target.value,getComputedStyle(document.documentElement).getPropertyValue('--bg').trim());$('#background').oninput=e=>theme(getComputedStyle(document.documentElement).getPropertyValue('--accent').trim(),e.target.value);$$('[data-preset]').forEach(b=>b.onclick=()=>{const p=b.dataset.preset;if(p==='ocean')theme('#24b7ff','#07131f');if(p==='light')theme('#1677a8','#eef6fb');if(p==='dark')theme('#65d5ff','#050b12')});
let fs=Number(localStorage.getItem('atlasFontScale')||1);function setFont(v){fs=Math.max(.9,Math.min(1.35,v));document.documentElement.style.setProperty('--font-scale',fs);localStorage.setItem('atlasFontScale',fs)}setFont(fs);$('#fontUp').onclick=()=>setFont(fs+.08);$('#fontDown').onclick=()=>setFont(fs-.08);$('#contrastBtn').onclick=()=>{document.body.classList.toggle('high-contrast');$('#contrastBtn').setAttribute('aria-pressed',document.body.classList.contains('high-contrast'))};$('#motionBtn').onclick=()=>{document.body.classList.toggle('reduce-motion');$('#motionBtn').setAttribute('aria-pressed',document.body.classList.contains('reduce-motion'))};
function speak(text){if(!('speechSynthesis'in window)){toast('Áudio não disponível neste navegador.');return}speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='pt-BR';u.rate=.92;speechSynthesis.speak(u)}$('#audioBtn').onclick=()=>{audioMode=!audioMode;$('#audioBtn').setAttribute('aria-pressed',audioMode);$('#audioBtn').textContent=audioMode?'🔊 Áudio ligado':'🔊 Áudio';toast(audioMode?'Leitura por áudio ativada.':'Leitura automática desativada.')};
$('#librasBtn').onclick=()=>{if(document.querySelector('[vw]')){toast('VLibras já está ativo.');return}const d=document.createElement('div');d.setAttribute('vw','');d.className='enabled';d.innerHTML='<div vw-access-button class="active"></div><div vw-plugin-wrapper><div class="vw-plugin-top-wrapper"></div></div>';document.body.appendChild(d);const s=document.createElement('script');s.src='https://vlibras.gov.br/app/vlibras-plugin.js';s.onload=()=>{new window.VLibras.Widget('https://vlibras.gov.br/app');toast('VLibras carregado. Use o ícone lateral.')};s.onerror=()=>toast('Não foi possível carregar o VLibras. Verifique a internet.');document.body.appendChild(s)};
$('#printBtn').onclick=()=>{toast('Abrindo versão otimizada. Escolha “Salvar como PDF”.');setTimeout(()=>window.print(),250)};
function openDB(){return new Promise((res,rej)=>{const r=indexedDB.open('atlas-torax',2);r.onupgradeneeded=e=>{const d=e.target.result;if(!d.objectStoreNames.contains('photos')){const s=d.createObjectStore('photos',{keyPath:'id'});s.createIndex('item','item',{unique:false})}};r.onsuccess=e=>{db=e.target.result;res()};r.onerror=()=>rej(r.error)})}function photos(item){return new Promise((res,rej)=>{const r=db.transaction('photos').objectStore('photos').index('item').getAll(item);r.onsuccess=()=>res(r.result.sort((a,b)=>a.time-b.time));r.onerror=()=>rej(r.error)})}function putPhoto(obj){return new Promise((res,rej)=>{const r=db.transaction('photos','readwrite').objectStore('photos').put(obj);r.onsuccess=res;r.onerror=()=>rej(r.error)})}function removePhoto(id){return new Promise((res,rej)=>{const r=db.transaction('photos','readwrite').objectStore('photos').delete(id);r.onsuccess=res;r.onerror=()=>rej(r.error)})}
async function optimizeImage(file){if(file.size<900000)return file;try{const bmp=await createImageBitmap(file);const max=2200,ratio=Math.min(1,max/Math.max(bmp.width,bmp.height)),w=Math.round(bmp.width*ratio),h=Math.round(bmp.height*ratio);const c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d',{alpha:false});x.drawImage(bmp,0,0,w,h);bmp.close();const blob=await new Promise(r=>c.toBlob(r,'image/jpeg',.88));return blob||file}catch(e){return file}}
async function savePhoto(item,file){const blob=await optimizeImage(file);const id=`${item}-${Date.now()}-${Math.random().toString(36).slice(2)}`;await putPhoto({id,item,blob,name:file.name,time:Date.now()})}
